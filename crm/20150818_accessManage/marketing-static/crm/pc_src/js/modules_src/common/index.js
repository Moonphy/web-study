 ...
 * 统一提示接口
 * @param msg 消息
 * @param type 类型，可选值 s:success e:error i:info
 * @param yes|options 成功回调函数|自定义参数不推荐使用
 */
$.alert = function (msg,type) {
    ...
    options = $.extend(true,options,{icon:type=='s'?'1':type=='e'?'5':type=='i'?'7':type});
    $.layer.alert(msg,options,yes);
};

$.msg = function (msg,type,time,callback) {
    var options = {icon:type=='s'?'1':type=='e'?'5':type=='i'?'7':type,time:time?time:3000};
    $.layer.msg(msg, options , callback?callback:undefined);
};

$.isEmpty = function (obj) {
    return !obj || $.trim(obj)=='' || obj==undefined || obj==null;
};

$.keyNotFound = function () {
    $.alert('抱歉，页面重要参数丢失，请退出重新进入！<br/> Tips: 请勿点击别人发送的链接地址，这可能会影响你的账户安全，请确保页面是程序自动跳转进来的，不要尝试删除网页链接中的参数！',
    'e',{
        btn:['我已了解此安全事项']
    });
};
