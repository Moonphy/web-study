...
function QiniuJsSDK(plupload, mOxie) {
    ...
    this.uploader = function(op) {
        ...
        that.uptoken_key = op.uptoken_key;
        that.domain_key = op.domain_key;
        that.token = '';
        that.domain = '';
        that.downloadUrl = op.downloadUrl;
            ...
            if (!op.uptoken) {
                ...
                ajax.onreadystatechange = function() {
                    if (ajax.readyState === 4 && ajax.status === 200) {
                        ...
                        that.token = res.model[that.uptoken_key || 'token'];
                        that.domain = res.model[that.domain_key || 'domain'];
                    }
                };
                ajax.send();
            } else {
                ...
                if (file) {
                    switch (err.code) {
                        ...
                        case plupload.FILE_SIZE_ERROR:
                            ...
                            errTip = '抱歉，门头照片每张最大可上传' + max_file_size;
                            break;
    ...
    this.getUrl = function(key) {
        ...
        key = encodeURI(key);
        var domain = this.downloadUrl?this.downloadUrl:this.domain?this.domain:'';
        return domain + key;
    };

    this.imageAve = function(key,callback) {
        var ajax = that.createAjax();
        ajax.open('GET', this.getUrl(key)+'?imageAve', true);
        ajax.onreadystatechange = function() {
            if (ajax.readyState === 4) {
                if (ajax.status === 200) {
                    var info = that.parseJSON(ajax.responseText);
                    callback(info);
                } else {
                    uploader.trigger('Error', {
                        status: ajax.status,
    ...
    this.imageInfo = function(key,callback) {
        if (!key) {
            return false;
        }
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                callback(that.parseJSON(xhr.responseText));
            }
        };
        xhr.send();
    };
