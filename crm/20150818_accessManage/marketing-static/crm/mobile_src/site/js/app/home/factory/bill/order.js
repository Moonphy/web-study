define(function(require,exports,module) {
  ...
  var List = require('../../../gallery/iscroll/list-api');
    ...
    var list = new List({
      ...
      url:'/wx/billOfDocument/find/orderMainList',
      buildParam:function(){
        var tmp = queryStr;
        if(!custId){
        ...
        Base.state.check(data,function(model){
          if(model){
              ...
              $.each(model.model,function(idx,mode){
                ...
                mode.state = BillStatus.order[mode.orderStatus] || '未知'+mode.state;
                mode.value = mode.money;
                mode.id = mode.orderMainID;
              });
              ...
              $('#wrapper').off(Base.events.click,'tr td.item').on(Base.events.click,'tr td.item',function(e){
                  location.href='order-detail.html?id='+$(this).parents('tr').attr('id');
              });
          }
        });//Base.state end
        return data.model;
